...
async fn test_http_integration_test() -> Result<()> {
  ...
  let wasm_bytecode = include_bytes!("wasm_test_apps/wasm_echo.wasm");
  let wasm_bytecode = general_purpose::STANDARD_NO_PAD.encode(wasm_bytecode);

  let response = hc.do_post("/wasm/echo", json!({
  ...
  let response = hc.do_get("/wasm/echo").await?;
  ...
  let response = hc.do_get("/wasm/echo/execute").await?;
  response.print().await?;
  assert_eq!(response.text_body()?, "1");
